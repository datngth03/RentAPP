...
export const registerService = ({ phone, name, password }) =>
   new Promise(async (resolve, reject) => {
      try {
         ...
         const token =
            response[1] &&
            jwt.sign({ id: response[0].id, phone: response[0].phone }, process.env.SECRET_KEY, {
               expiresIn: "3d",
            });
         resolve({
            err: token ? 0 : 2,
            message: token ? "regist is successfully" : "phone number existed",
            token: token || null,
         });
      } catch (err) {
         reject(err);
